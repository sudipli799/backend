...
const getCommentsWithReplies = async (req, res) => {
  try {
    ...
    if (!post_id) {
      return res.status(400).json({ status: false, message: 'post_id is required' });
    }

    // Recursive function to build nested replies
    const buildReplyTree = async (parentId) => {
      const replies = await Comment.find({ parent_comment_id: parentId, is_deleted: 0 }).sort({ createdAt: -1 });
      const replyTree = [];
